...
import { TASK, main } from '@ai-devs-4/s02e05';
...
export const s02e05Router = Router();

s02e05Router.post('/run', async (_req, res): Promise<void> => {
  const steps: LogEntry[] = [];
  const log = (message: string, level: LogEntry['level'] = 'info'): void => {
    steps.push({ message, level });
    console.log(`[s02e05/${level}] ${message}`);
  };

  try {
    log('Starting S02E05 — Drone Strike...');
    log('Analyzing terrain map and programming drone...');
