...
async function execute(addLog: AddLog): Promise<void> {
  addLog('Starting S02E02 — Electricity Grid Puzzle...', 'info');

  const res = await fetch(`${BACKEND_URL}/api/lessons/s02e02/run`, {
    method: 'POST',
...
registerLesson({
  id: 'S02E02',
  title: 'Electricity Grid Puzzle',
  description: 'Solve a 3×3 cable grid by rotating tiles to connect the power source to all plants.',
  execute,
});
