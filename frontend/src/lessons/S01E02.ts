...
async function execute(addLog: AddLog): Promise<void> {
  addLog('Starting S01E02 — Track Suspect Near Power Plant...', 'info');

  const res = await fetch(`${BACKEND_URL}/api/lessons/s01e02/run`, {
    method: 'POST',
...
registerLesson({
  id: 'S01E02',
  title: 'Track Suspect',
  description: 'Find suspect near power plant',
  execute,
});
